...
class JoltControls extends EventDispatcher {
    ...
    update() {
        ...
        // 物理引擎计算物体位置
        {
            ...
            this.world.Step(delta, numSteps);
            for (var i = 0, il = this.moveObjs.length; i < il; i++) {
                var objThree = this.moveObjs[i];
                var objPhys = objThree.userData.physicsBody
                objThree.position.copy(wrapVec3(objPhys.GetPosition()));
                objThree.quaternion.copy(wrapQuat(objPhys.GetRotation()));
